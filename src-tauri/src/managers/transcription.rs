...
use crate::settings::{
    get_settings, ModelUnloadTimeout, OrtAcceleratorSetting, TranscriptionProviderConfig,
...
impl TranscriptionManager {
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        // Skip for local Whisper models since custom words are already passed as initial_prompt.
        let is_local_whisper = matches!(
            settings.transcription_config,
            TranscriptionProviderConfig::LocalProvider(_)
        ) && self
            .model_manager
        ...
        let corrected_result = if !settings.custom_words.is_empty() && !is_local_whisper {
            apply_custom_words(
                &transcription_text,
